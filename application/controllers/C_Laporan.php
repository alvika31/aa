...
class C_Laporan extends CI_Controller
{
    ...
    function filter()
    {
        ...
        $data = [
            'laporan' => $this->M_Laporan_model->filterLaporan($tanggal)->result(),
            'total' => $this->M_Laporan_model->jumlahHargaFilter($tanggal),
    ...
    {
        $tanggal_1 = $this->input->post('date_1');
        $tanggal_2 = $this->input->post('date_2');
        if ($this->input->post('submit')) {
            if ($tanggal_1 == $tanggal_2) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">
                Harap Bedakan Tanggal
              </div>');
                redirect('C_Laporan');
            } elseif ($tanggal_1 > $tanggal_2) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">
                ...
                $pdf->SetFont('Arial', 'B', 12);
                $pdf->Cell(0, 7, 'Periode ' . $tanggal_1 . ' - ' . $tanggal_2, 0, 1, 'C');
