...
class M_Cucimobil_model extends CI_Model
{
    ...
    {
        $last = $this->db->select('cucimobil_idtransaksi')->from('tbl_cucimobil')->order_by('cucimobil_id', 'DESC')->get()->row();
        $last_urnno = $last->cucimobil_idtransaksi;
        ...
        $urnno = "TR-0{$new}";
        $data = array(
            'cucimobil_idtransaksi' => $urnno,
            'cucimobil_tanggal' => date('Y-m-d'),
